...
(function () {
  ...
  function petInstantHatchCost(pet) {
    ...
    const remaining = pet.incubationStartedAt && pet.incubationEndsAt ? Math.max(1, Math.ceil((pet.incubationEndsAt - Date.now()) / 60000)) : (pet.incubationMinutes || 1);
    return Math.max(6, Math.ceil(remaining / 8));
  }
  ...
  function completeTask(taskId, fromPomodoro) {
    ...
    if (beforeMood && afterMood && afterMood.level < beforeMood.level) {
      const moodText = "心情从「" + beforeMood.name + "」下降到「" + afterMood.name + "」。";
  ...
  function drawCharacterPixels(sctx, appearance, size) {
    ...
    if (appearance.bottomStyle === "旅行短裤") {
      ...
      fillRect(sctx, 11 * unit, 25 * unit, 10 * unit, 1 * unit, "#b8956e");
...
function getTreasureSprite(size) {
  ...
  function renderAdventurePage(targetCtx, view, y) {
    ...
    ADVENTURE_LOCATIONS.forEach(function (location, index) {
      ...
      drawButton(targetCtx, buttonX, cardY + 56, 218, 78, buttonLabel, canGo ? "paper" : "disabled", { small: true, disabled: !canGo });
      if (canGo) registerRegion(mainRegions, buttonX, cardY + 56, 218, 78, function () { startAdventure(location.id); });
    });
