...
(function () {
  ...
  const ACHIEVEMENTS = [
    ...
    { id: "collection_pet_3", category: "收集类", logicKey: "collection_pet_3", name: "小队成形", description: "身边已经有一支三人（兽）小队陪你冒险。", condition: "拥有 3 只已孵化的宠物。", rewardText: "金币 +100" },
    ...
    { id: "adventure_pet_hatched", category: "冒险类", logicKey: "adventure_pet_hatched", name: "第一个伙伴", description: "蛋壳里蹦出来的，不只是宠物，也是你的同行者。", condition: "成功孵化 1 个宠物。", rewardText: "金币 +30" },
    { id: "adventure_pet_adult", category: "冒险类", logicKey: "adventure_pet_adult", name: "一路养到成年", description: "从一颗小蛋到成年伙伴，你们一起走完了成长条。", condition: "至少有一只宠物从幼年成长为成年阶段。", rewardText: "金币 +80" },
    ...
    { id: "misc_first_rest", category: "其他", logicKey: "misc_first_rest", name: "第一次放松", description: "该歇就歇，恢复状态再上路。", condition: "第一次购买任意一个休息恢复。", rewardText: "金币 +20" },
    { id: "misc_rest_chill", category: "其他", logicKey: "misc_rest_chill", name: "爱你老己！", description: "偶尔豪华摆烂一下，也是在认真生活。", condition: "享受一次「豪华摆烂」。", rewardText: "金币 +30" },
  ...
  function normalizeState(input) {
    ...
    // 双手武器迁移：旧存档只有 weapon（默认右手），盾牌类归左手
    if (!stateValue.profile.appearance.weapon) stateValue.profile.appearance.weapon = "无";
    ...
    const legacyWeapon = stateValue.profile.appearance.weapon;
    if (!stateValue.profile.appearance.weaponLeft || stateValue.profile.appearance.weaponLeft === "无") {
      if (isShieldWeaponValue(legacyWeapon)) stateValue.profile.appearance.weaponLeft = legacyWeapon;
    }
    if (!stateValue.profile.appearance.weaponRight || stateValue.profile.appearance.weaponRight === "无") {
      if (!isShieldWeaponValue(legacyWeapon)) stateValue.profile.appearance.weaponRight = legacyWeapon;
    }

    // 兜底：若不在已拥有的 weapons 列表里，则回退为“无”
    ...
    // 双手武器：确保已拥有装备中始终包含“无”（空手选项），同时保留存档里的已有装备
    if (!stateValue.collection.weapons || !stateValue.collection.weapons.length) stateValue.collection.weapons = ["无", "木剑"];
    else if (stateValue.collection.weapons.indexOf("无") < 0) stateValue.collection.weapons.unshift("无");

    if (!stateValue.profile.appearance.weaponLeft) stateValue.profile.appearance.weaponLeft = "无";
    if (!stateValue.profile.appearance.weaponRight) stateValue.profile.appearance.weaponRight = stateValue.profile.appearance.weapon || "无";

    if (!stateValue.collection.weapons || stateValue.collection.weapons.indexOf(stateValue.profile.appearance.weaponLeft) < 0) stateValue.profile.appearance.weaponLeft = "无";
    if (!stateValue.collection.weapons || stateValue.collection.weapons.indexOf(stateValue.profile.appearance.weaponRight) < 0) stateValue.profile.appearance.weaponRight = "无";
  ...
  function openCreateChallengeModal() {
    ...
    setModal({ type: "createChallenge", title: "", bucket: "todo", difficulty: "normal" });
  ...
  function confirmCreateChallenge() {
    ...
    const firstCreated = state.tasks.length === 0;
    state.tasks.push(createTask(title, bucket, difficulty, false, false));
    addLog("新增了一个" + taskBucketLabel(bucket) + "：“" + title + "”。");
  ...
  function recoverWith(optionId) {
    ...
    const beforeMood = currentHeroMood();
    const firstRest = !activeAchievements().unlocked || !activeAchievements().unlocked.misc_first_rest;
    ...
    if (firstRest) unlockAchievement("misc_first_rest");
    if (option.id === "rest-chill") unlockAchievement("misc_rest_chill");
    const afterMood = currentHeroMood();
    if (afterMood && beforeMood && afterMood.level !== beforeMood.level) {
  ...
  function drawCharacterPixels(sctx, appearance, size) {
    ...
    let __rightWeapon = appearance.weaponRight || (!isShieldWeaponValue(__prevWeapon) ? __prevWeapon : "无");
    // 防御性处理：若右手错误存了盾牌，则强制移到左手
    ...
    // 恢复右手绘制所临时修改的 weapon 字段，并额外绘制左手盾牌
...
function getTreasureSprite(size) {
  ...
  function getCatalogPreview(item, size) {
    const previewAppearance = {
      ...
      weapon: item.category === "item" ? item.unlockValue : "无",
  ...
  function renderRewards(targetCtx, x, y, width) {
    ...
    drawText(targetCtx, "奖励区", shellX + 26, shellY + 18, 22, THEME.ink, "700");
    drawText(targetCtx, "可兑换奖励", shellX + 26, shellY + 46, 16, THEME.inkSoft, "600");
  ...
  function renderRest(targetCtx, x, y, width) {
    ...
    const shellH = sectionHeight - 20;
    const shell = drawSectionShell(targetCtx, shellX, shellY, shellW, shellH, {});
    drawText(targetCtx, "休息恢复", shellX + 26, shellY + 18, 22, THEME.ink, "700");
    drawText(targetCtx, "花金币回血恢复状态", shellX + 26, shellY + 46, 16, THEME.inkSoft, "600");
    let cursor = shellY + shell.barH + 20;
    REST_OPTIONS.forEach(function (option) {
      drawCard(targetCtx, shellX + 18, cursor, shellW - 36, cardHeight, THEME.paper);
      drawText(targetCtx, option.name + "  •  " + option.price + " 金币", shellX + 38, cursor + 26, 28, THEME.ink, "700");
      drawText(targetCtx, "恢复 " + option.hp + " HP 与 " + option.exp + " EXP。 " + option.description, shellX + 38, cursor + 82, 20, THEME.inkSoft, "500");
      const enough = state.profile.gold >= option.price;
      drawButton(targetCtx, shellX + shellW - 136, cursor + 24, 96, 68, enough ? "休息" : "不够钱", enough ? "yellow" : "disabled");
      if (enough) registerRegion(mainRegions, shellX + shellW - 136, cursor + 24, 96, 68, function () { recoverWith(option.id); });
      cursor += cardHeight + 18;
    });
    return y + sectionHeight;
  }
  ...
  function renderTasksPage(targetCtx, view, y) {
    ...
    const tabBarH = 106;
    const challengeContentH = bucketSectionHeight(visibleTodo.length) + 16 + bucketSectionHeight(groups.habit.length) + 16 + bucketSectionHeight(groups.learn.length) + 40;
    const restContentH = (function () {
      const cardHeight = 132;
      const hasRewards = state.rewards.length > 0;
      const rewardsH = hasRewards ? 120 + state.rewards.length * (cardHeight + 18) : 236;
      const restH = 120 + REST_OPTIONS.length * (cardHeight + 18);
      return rewardsH + 18 + restH + 24;
    })();
    ...
    drawSectionHeader(targetCtx, x + 30, y + 30, "任务区", tab === "challenge" ? "挑战列表" : "休整与恢复");
  ...
  function renderShopPage(targetCtx, view, y) {
    ...
    const sectionHeight = 244 + rowsPerPage * 168 + 80;
    drawPanel(targetCtx, x, y, w, sectionHeight, THEME.paper);
    ...
    pageItems.forEach(function (item, index) {
      ...
      const cardY = cursor + row * 168;
      drawCard(targetCtx, cardX, cardY, cardW, 148, THEME.paper);
      ...
      drawText(targetCtx, item.name + "  •  " + item.price + " 金币", textX, cardY + 76, 28, THEME.ink, "700");
      drawText(targetCtx, item.description, textX, cardY + 120, 20, THEME.inkSoft, "500");
      let label = "购买";
      let variant = "yellow";
  ...
  function renderCreateChallengeModal(targetCtx, view) {
    ...
    drawText(targetCtx, "待办 / 习惯 / 技能", x + 34, y + 78, 16, "#d8aa95", "500");
    ...
    const rowY = y + 116;
    drawCard(targetCtx, left, rowY, innerW, 234, THEME.paper);
    drawInputField(targetCtx, left + 18, rowY + 18, innerW - 36, 72, "挑战名称", getFieldValue("createChallengeTitle"), "例如：整理桌面", "createChallengeTitle", false);

    const dual = innerW >= 560;
    const fieldW = dual ? Math.floor((innerW - 36 - 18) / 2) : (innerW - 36);
    const secondRowY = rowY + 126;
    drawSelectorField(targetCtx, left + 18, secondRowY, fieldW, 72, "类型", (TASK_BUCKET_OPTIONS[modal.bucket] || TASK_BUCKET_OPTIONS.todo).selector, function () {
      const order = ["todo", "habit", "learn"];
      const idx = Math.max(0, order.indexOf(modal.bucket));
      modal.bucket = order[(idx + 1) % order.length];
      renderApp();
    });
    drawSelectorField(targetCtx, left + 18 + (dual ? fieldW + 18 : 0), secondRowY + (dual ? 0 : 104), fieldW, 72, "难度", DIFFICULTIES[modal.difficulty] ? DIFFICULTIES[modal.difficulty].label : DIFFICULTIES.normal.label, function () {
      const keys = Object.keys(DIFFICULTIES);
      const current = keys.indexOf(modal.difficulty);
      modal.difficulty = keys[clamp(current + 1, 0, keys.length - 1)] || "normal";
      if (current >= keys.length - 1) modal.difficulty = keys[0] || "normal";
      renderApp();
    });
  ...
  function renderEditorModal(targetCtx, view) {
    ...
    rowY += 108;
    drawEditorChoiceRow(targetCtx, controlX, rowY + contentShift, controlW, "weapons", "已拥有装备", state.collection.weapons.map(function (value) { return { label: value, width: compactEditor ? 120 : 160, active: value === "无" ? (state.profile.appearance.weaponLeft === "无" && state.profile.appearance.weaponRight === "无") : (state.profile.appearance.weaponLeft === value || state.profile.appearance.weaponRight === value), onClick: function () { equipAppearanceFromCollection("weapon", value); } }; }));
    rowY += 108;
